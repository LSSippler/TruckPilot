...
- [[01-Phases/Phase-5.29-Optional-SCS-SDK-Controller]] — **COMMITTED 2026-05-23**, Live-Test pending: DLL-Extension (`scs_input_init` + SHM `Local\TruckPilotControls` + per-frame `input_event_cb`) + Plugin `scs-sdk-output` (PostPhase, Safety-Guards). 24 Tests grün. `plugins/scs_sdk_output.dll` deployed. Commit `b997781c`.
...
- [[01-Phases/Phase-5.29-C-Lane-Detection-Plugin]] — **COMMITTED 2026-05-23**, Live-Test pending: Lane-Detection Plugin (onnx-cpu, 37ms, PhaseB). on_load-Probe leitet Row/Col-Anchors aus Output-Shape ab (nicht als const). enabled=false Default. SCHEMA-LOCK: lane.center_offset/confidence/detection_count/left_visible/right_visible. 19 Tests grün. `truckpilot_plugin_lane_detection.dll` deployed.
- [[01-Phases/Phase-5.29-D-LaneKeeper-Vision-Rewrite]] — **CLOSED 2026-05-23**: Lane-Keeper Vision-Rewrite. 5-Level Fallback Cascade (DS1). LaneKeeperMode (RouteFollowing/Vision/Off), fallback.rs + extrapolation.rs + heading_hold.rs. VisionLostLongBlackout FailureReason, L4-Disengage-Trigger in state_machine. 76 lane-keeper + 119 core Tests grün. Commit `66a4e254`.
...
- [[01-Phases/Phase-0c.2-Arc-Length-Parametrisierung]] — **CLOSED 2026-05-25**: Arc-Length LUT (36B/Segment, GL8-Quadratur), Newton-Raphson t_at_arc_length, cross-segment lookahead(15m). 21/21 Tests grün, p99=0.40µs, LUT=44.8 MB, alle STOP-Bedingungen PASS. Commit-ready.
- Phase 0d.1a (Audit) — **CLOSED 2026-05-25** (Read-only): 12 fehlende Sektoren — alle bezier_patch (0x27) Failures. Root Cause: Vegetation 3×11=33 statt 4×11=44 Bytes → 11-Byte Under-Read. 6 862 Items verloren, 4 Kern-Routing-Sektoren betroffen (Berlin, Ostdeutschland, Norddeutschland, Rheintal). Fix: 1 Zeile `3 * 11` → `4 * 11`. Report: `outputs/2026-05-25/cc_phase_0d_1a_missing_sectors_audit.md`.
- Phase 0d.2 (CC-Diagnose) — **CLOSED 2026-05-25** (Read-only): 9/40 Städte in Largest CC (343k Nodes, 29.8%). Routing-Success 4.6% = 36/780 Paare = 9×8/2 — exakte mathematische Erklärung. 31 Städte in Singletons/Mini-Gruppen, keine Cross-Component-Edges zwischen Top-10 CCs. München + Hamburg = Singletons trotz 15m Snap-Distanz. Root Cause: fehlende bidirektionale Prefab-Connections, nicht Algo-Problem. Report: `outputs/2026-05-25/cc_routing_diagnosis.md`.
- Phase 0d.2a (City-Snap-Fix) — **CLOSED 2026-05-25**: Snap-Radius 5km→20km + global fallback. Alle 40 Städte snappen jetzt. 7 neue Nodes (Madrid, Valencia, Lissabon, Porto, Oslo, Goeteborg, Bergen) — alle in isolierten DLC-CCs außerhalb CC-0. Kein echter Routing-Gewinn (72→72 echte Paare). Lissabon+Porto False-Positive: gleicher Node (24.7km Abstand = Koordinaten-Problem). Report: `outputs/2026-05-25/city_snap_fix_report.md`.
- Phase 0d.2b (Pass-2 Cross-Sector) — **CLOSED 2026-05-25** ⛔ STOP (0% CC-Wachstum): Spatial-Match Pass 2 (200m/15m/adj-sector) implementiert. Ergebnis: 0 neue Edges. Ursache: Nur 4 Orphan-Endpoints in 366k Straßen — die 601k isolierten Nodes sind Ghost-Nodes (nicht Orphan-Endpoints). Spatial Matching kann Ghost-Nodes nicht verbinden. Code fertig (spatial_match.rs + graph.rs, 137 Tests grün), aber nutzlos ohne bezier_patch Fix. Report: `outputs/2026-05-25/pass2_report.md`.
- [[01-Phases/Phase-0d.3a-CitySii-Parser]] — **CLOSED 2026-05-25**: city.sii SII-Text-Parser + route-test `--scs-dir` Integration. 9/9 Unit-Tests grün. Baseline: 156/1560 (10.0%) toml-only. Post-Messung mit echten Archiven ausstehend. Reviewer-Fixes: O(N²)→O(N) display_idx, positions-HashMap einmalig. STOP 5km. Report: `outputs/2026-05-25/city_sii_parser_report.md`.
- [[01-Phases/Phase-0d.3a-Sprint1-Verification]] — **CLOSED 2026-05-25**: Live-Run mit ETS2 1.55 Archives. STOP: ETS2 1.48+ hat `position:`-Feld aus `.sui` City-Dateien entfernt. 384 `.sui`-Dateien gefunden, 0 Koordinaten extrahierbar. Parser-Infrastruktur korrekt (alt+neu Format, HashFS-Directory-Listing, Merge-Semantik). Routing 10.0% = Baseline, keine Regression. Report: `outputs/2026-05-25/city_sii_liverun_report.md`.
- [[01-Phases/Phase-1a-LaneFollower-Skeleton]] — **CLOSED 2026-05-25**: Lane-Follower Plugin-Crate erstellt. SplineIndex-Loading in on_load(), 8 BB-Diagnostik-Keys @ 50 Hz, STOP-Conditions (dist > 50m → dist_warn, heading-diff > 90° → heading_warn), tick_request → None (Phase 1a). 12/12 Tests grün, 0 Warnungen. Auto-Deploy via xtask-Prefix-Erkennung. Report: `outputs/2026-05-25/phase_1a_lane_follower_skeleton.md`.
- [[01-Phases/Phase-1b-SplineFollower]] — **CLOSED 2026-05-25**: Arc-Length-Lookahead (15m, cross-segment, Heading-Filter cos 60°), Stabilitätsmetrik (50-Tick Ringpuffer), 8 neue BB-Keys, 3-State `lookahead_status` (ok/dead_end/iteration_limit). `LookaheadResult.iteration_count` + `LOOKAHEAD_MAX_HOPS pub` in arc_length.rs. 17/17 Tests grün, 0 Warnungen. Report: `outputs/2026-05-25/phase_1b_spline_follower.md`.
- [[01-Phases/Phase-1c-Pure-Pursuit-Steering]] — **CLOSED 2026-05-25**: Pure-Pursuit Steering (erster ControlRequest). `pure_pursuit.rs`: `compute_steering()`, WHEELBASE_M=4.0m. tick_request() gibt Some(CR{steering}) in Active+ok, None in Observer/dist_warn/heading_warn. EMA α=0.3. 3 neue BB-Keys (steering_cmd/curvature/filtered). 41/41 Tests grün, 0 Warnungen.
- [[01-Phases/Phase-graph-rs-Heading-Fix]] — **CLOSED 2026-05-25**: `graph.rs:51-52` ETS2-Heading-Konvention-Fix. `heading.sin()/.cos()` → `heading_rad = -heading * TAU`. Doc-Comment. 3 Router-Tests FRAC_PI_2→0.75. 1 Regressionstest (heading=0.5 Süd). 13/13 + 34/34 Tests grün.
- [[01-Phases/Phase-P0.1-Heading-Filter]] — **CLOSED 2026-05-24**: P0.1 Heading-aware Nearest. `nearest_with_heading_filter()` in spline_index.rs (dot ≥ 0.5, Fallback auf nearest-geometric). `NearestHit.heading_filter_applied` Diagnostik-Flag. lane-follower nutzt neue Methode. 2 neue spline_index-Tests + `lane_follower.heading_filter_applied` BB-Key. 149/149 + 42/42 Tests grün. Erwartet: heading_warn auf bidirektionalen Straßen von 47%→~0%.
- [[01-Phases/Phase-P0.2-Soft-Degrade]] — **CLOSED 2026-05-24**: P0.2 Soft-Degrade EngageMode. `HeadingUnrecoverable` Hard-Fault entfernt. Neues `EngageMode` Sub-State-Machine (Route/Lane/Degraded) mit Hysterese-Timern (150/100/250/100/1500 Ticks). Vision-Bypass. HeadingStage Disengaging-Latch-Reset via BB-Signal. 142/142 Tests grün (+10 P0.2, -2 heading-fault).
- [[01-Phases/Phase-P0.3-Lane-Follower-EngagedLane]] — **CLOSED 2026-05-25**: P0.3 ENGAGED_LANE + DEGRADED Mode. tick_request() gate auf autopilot.engage_mode (route/lane→CR, degraded→None). lane_follower.engage_source BB-Key. lane_only_engage State-Machine-Feld. engage-cli --lane-only Flag (bypasses router.active). 147/147 + 47/47 Tests grün.
- [[01-Phases/Phase-P3.1-JunctionDetector-Foundation]] — **CLOSED 2026-05-26**: P3.1 JunctionDetector Foundation. detect_junction() (O(n), 25m snap, degree≥2 + heading-spread>30°). 3-Frame-Aktivierung, 5-Frame-Coast. 4 neue BB-Keys (junction_detected/phase/distance_m/max_degree). RouterGraph parallel zu SplineIndex aufgebaut. 55/55 Tests grün. Commit `875a4996`.
- [[01-Phases/Phase-P0.4-Lane-Offset-QW1]] — **CLOSED 2026-05-26**: P0.4 Lane-Offset QW1. Lookahead-Punkt +1,875 m nach rechts (halbe ETS2-Fahrspurbreite). evaluate_tangent() an la.seg_idx/t → Rechts-Normalvektor. BB-Key-Override `plugin.lane-follower.lane_offset_m`. 6 neue Diagnostik-Keys. 58/58 Tests grün (+3 neu, +2 angepasst für STOP-Condition).
...
- [[01-Phases/Phase-Big8-cruise-decouple]] — **CLOSED 2026-05-31**: ETS2-CC vollständig entkoppelt. cruise_active-Precondition + CruiseDeactivated-Fault + cruise_off_ticks-Debounce entfernt. Speed-Controller-Fallback 80→50 km/h. 177 Tests grün.
- [[01-Phases/Phase-Big8b-LaneKeeper-Heading-Fix]] — **CLOSED 2026-05-31**: Lane-Keeper Heading-Konversion. `t.heading` (ETS2 [0..1] CCW) korrekt in CW-Radiant konvertiert. Phantom-49°-Fehler beseitigt der `lk_skip=heading_stage` für gesamte Fahrt verursacht hat. 81 Tests grün, 1 neuer Regressionstest.

- [[01-Phases/Phase-2b-NavCurve-SharedSplineIndex]] — **CLOSED 2026-05-31**: NavCurves in Arc-geteilten SplineIndex. 1.303M→1.435M Segs, 2 Kopien→1. `PluginContext.spline_index` neu. Lane-Follower on_load shared-Pfad. 79/79 Tests grün, Clippy clean.

- [[01-Phases/Phase-2a-Edge-UID-Mapping-Audit]] — **CLOSED 2026-05-31**: Edge-UID-Mapping-Audit (Opt-2 Voraussetzung). 1303542 Edges, **100% Match-Rate** für alle Typen. (from_uid,to_uid)-Lookup eindeutig. NavCurves in graph.json vorhanden (131571) aber NICHT im HUD-SplineIndex. Verdikt: **a) grünes Licht für Phase 2b.**
...
- [[01-Phases/Phase-5.29-Optional-SCS-SDK-Crash-Fix]] — **CLOSED 2026-05-23**: `ScsInputDevice` 48→56 Byte fix. Fehlender `input_active_callback` + falsche Feldreihenfolge + fehlender `flags`-Parameter in Callback-Signatur → ETS2 Crash bei Karriere-Auswahl. Compile-Time Size-Asserts ergänzt. 13/13 Tests grün. `30612c0`.
- [[01-Phases/Phase-5.29-CC-Engage-Mode-Aware]] — **COMMITTED 2026-05-23**, Live-Test pending: `EngagementPreconditions` mode-aware. Vision-Mode: engage ohne Route möglich, gate = `telemetry_fresh && truck_on_road && heading_ok_for_engage && speed_ok && lane_keeper_engage_allowed`. `check_preconditions()` Vision-Mode: `router_active = lane_keeper.engage_allowed`. Mode-Switch mid-Active → clean Off. EngagementChecklist.tsx mode-aware. 126/126 Tests grün.
...
## Map-Parser Spike-Phasen (Phase 0b/0c/0c.2 — Arc-Length Pre-Spike)

- [[01-Phases/Phase-0b-Hermite-Spline-Generator]] — **CLOSED 2026-05-24**: Hermite-Spline-Generator aus ETS2-Routing-Graph. spline.rs (HermiteSegment 80B, atan2(dx,-dz) Heading, Catmull-Rom-Tangenten, Degree-Fix via HashSet). spline_gen CLI (gen/plot/stats). 3 Eyeball-SVGs. 19 Unit-Tests grün. 1.303M Seg., 104 MB.
- [[01-Phases/Phase-0c-Rtree-Spatial-Index]] — **CLOSED 2026-05-24**: R-tree Spatial Index (rstar bulk_load). spline_index.rs: SegmentAabb (chord + sampled), Newton-Projektion, nearest/within_radius/nearest_with_projection. bench-Subcommand. 13 Tests grün. nearest p99 <<5ms.
- [[01-Phases/Phase-0c.2-Arc-Length-Parametrisierung]] — **CLOSED 2026-05-24**: Arc-Length LUT (GL8-Quadratur, 36B/Seg., 47MB gesamt). t_at_arc_length (BinSearch+Newton). Cross-Segment-Lookahead (cos60° Heading-Filter). arclength-Subcommand. 21 Tests grün. u16-Fehler <0.031m bei 2km. Pure-Pursuit-ready.
