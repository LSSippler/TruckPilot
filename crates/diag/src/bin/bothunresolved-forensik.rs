...
fn scan_sector(data: &[u8], target: &FxHashSet<u64>, apply_filter: bool) -> FxHashSet<u64> {
    ...
    while pos <= last {
        ...
        if target.contains(&candidate)
            && (!apply_filter || is_plausible_node_definition(data, pos))
        {
...
fn main() -> Result<()> {
    ...
    let t = Instant::now();
    let audit_raw: Vec<AuditEvent> = serde_json::from_reader(
    ...
    for e in &consistent {
        ...
        let sub = if bucket == Bucket::BothFound {
            ...
            let road_plus_both =
                shared.iter().any(|p| road_set.contains(p));
            if road_plus_both {
...
fn write_markdown(
    ...
    w!("| Metric | Value |");
    w!("|---|---|");
    w!("| BothUnresolved events (input) | {} |", inconsistent.len() + total_classified);
    w!("| InputInconsistency (UID in graph.json) | {} |", inconsistent.len());
    w!("| Classifiable events | {} |", total_classified);
    w!("| Sectors discovered | {} |", discovered.len());
    w!("| Global node-map size | {} |", global_uids.len());
    w!("| Scan duration | {:.1}s |", scan_duration_ms as f32 / 1000.0);
    w!("| Total duration | {:.1}s |", total_duration_ms as f32 / 1000.0);
    w!();
    ...
    if inconsistent.is_empty() {
        w!("✅ All {} BothUnresolved events are consistent with `graph.json`.",
            total_classified);
        w!();
    } else {
        w!("⚠ **{} events excluded as InputInconsistency** — these BothUnresolved",
            inconsistent.len());
        ...
        for (e, a, b) in inconsistent.iter().take(20) {
            w!(
                "| {:?} | {:?} | {} | {:?} | {} |",
                e.item_uid, e.node_a, a, e.node_b, b
            );
        }
    ...
    if both_found > 0 {
        ...
        w!(
            ...
            pct(sub_counts[SubBucket::RoadPlusBothSameSector as usize], both_found)
        );
        w!();
    }
    ...
    w!();
    for b in [Bucket::BothMissing, Bucket::OnlyAFound, Bucket::OnlyBFound, Bucket::BothFound] {
    ...
    w!("## 6. Sample Events per Bucket (up to {})", args.sample_limit);
    w!();
    for b in [Bucket::BothMissing, Bucket::OnlyAFound, Bucket::OnlyBFound, Bucket::BothFound] {
        let samples: Vec<&ForensicResult> =
            results.iter().filter(|r| r.bucket == b).take(args.sample_limit).collect();
        if samples.is_empty() {
            continue;
        }
        w!("### {:?} (showing {} of {})", b, samples.len(), counts[b as usize]);
        w!();
        ...
        for r in samples {
            let a_sec = if r.node_a_sectors.is_empty() {
                ...
                r.node_a_sectors.iter().take(2).map(|s| short_sec(s)).collect::<Vec<_>>().join(", ")
            };
            let b_sec = if r.node_b_sectors.is_empty() {
                "—".to_string()
            } else {
                r.node_b_sectors.iter().take(2).map(|s| short_sec(s)).collect::<Vec<_>>().join(", ")
            };
            let r_sec = if r.road_origin_sectors.is_empty() {
                "—".to_string()
            } else {
                r.road_origin_sectors.iter().take(2).map(|s| short_sec(s)).collect::<Vec<_>>().join(", ")
            };
            w!(
                "| {:?} | {} | {} | {} | {} | {} |",
                r.road_uid, r.node_a, r.node_b, a_sec, b_sec, r_sec
            );
        }
        w!();
    }
...
fn write_focus_city(
    ...
    let Some(&(_, cx, cz)) = cities.iter().find(|(n, _, _)| n.eq_ignore_ascii_case(city_name))
    else {
    ...
    let in_radius: Vec<&ForensicResult> = results
        ...
        .collect();
    w!("- BothUnresolved roads with origin sector in 10km radius: **{}**",
        in_radius.len());
    ...
    for (s, c) in sec_vec.iter().take(30) {
        ...
        w!("| `{}` | {} | {} | {} | {} | {} |", short_sec(s), c[0], c[1], c[2], c[3], total);
    }
    w!();
...
fn write_diagnosis(
    ...
    if bm > 60.0 {
        ...
        w!();
        w!("{:.1}% of classifiable BothUnresolved roads have endpoint UIDs that are",
            bm);
        ...
        if same > 50.0 {
            ...
            w!();
            w!("{:.1}% of BothUnresolved roads have both endpoint UIDs in sector data", bf);
            w!("({:.1}% in the SAME sector). Parser extracts the nodes correctly,", same);
            ...
            w!();
            w!("{:.1}% BothFound, but only {:.1}% in same sector — cross-sector",
                bf, same);
        ...
        w!();
        w!("{:.1}% OnlyAFound + {:.1}% OnlyBFound — one endpoint UID per road",
            oa, ob);
        ...
        w!("**ROOT=MIXED**");
        w!();
        w!("No dominant bucket: BothMissing={:.1}%, BothFound={:.1}%, OnlyOne={:.1}%.",
            bm, bf, oa + ob);
...
fn write_json(
    ...
    let pct = |n: usize, d: usize| if d == 0 { 0.0f64 } else { n as f64 * 100.0 / d as f64 };
