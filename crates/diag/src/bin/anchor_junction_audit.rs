...
fn extract_anchors(item_bytes: &[u8], item_type: u32) -> Vec<u64> {
    ...
    match item_type {
        T_VIS_AREA => {
            if let Some(u) = read_u64(item_bytes, KDOP_END) {
                uids.push(u);
            }
        }
        T_SIGN | T_MAP_OVERLAY => {
            if let Some(u) = read_u64(item_bytes, KDOP_END + 8) {
                uids.push(u);
            }
        }
        T_FAR_MODEL => {
            if let Some(u) = read_u64(item_bytes, KDOP_END + 24) {
                uids.push(u);
            }
        }
        T_BUS_STOP => {
            for off in [KDOP_END, KDOP_END + 8, KDOP_END + 16] {
                if let Some(u) = read_u64(item_bytes, off) {
                    uids.push(u);
                }
            }
        }
        T_MODEL => {
            if let Some(n) = read_u32(item_bytes, KDOP_END + 24) {
                ...
                if let Some(u) = read_u64(item_bytes, node_off) {
                    uids.push(u);
                }
            }
        }
...
fn main() {
    ...
    for (path, idx) in &sector_paths {
        ...
        if parse_sector(&data).is_err() {
            continue;
        }
        sectors_audited += 1;
        let report = audit_sector(&data);
        for it in &report.items {
            if !target.contains(&it.item_type) {
                continue;
            }
        ...
        *combo_counts.entry(combo).or_insert(0) += 1;
        if types.len() >= 2 {
            ...
            if known {
                ...
                if deg == 0 && !in_prefab {
                    junction_singletons += 1;
                } else {
                    junction_non_singletons += 1;
                }
            } else {
    ...
    let _ = writeln!(
        out,
        "SECTION 2: ANCHOR-DISTRIBUTION (Nodes by # referencing items)"
    );
    let _ = writeln!(
        out,
        "-------------------------------------------------------------"
    ...
    let _ = writeln!(
        ...
        "1                | {:>12} | {:>5.2}%",
        nodes_with_1,
        pct(nodes_with_1 as u64, total_unique as u64)
    );
    let _ = writeln!(
        out,
        "2                | {:>12} | {:>5.2}%",
        nodes_with_2,
        pct(nodes_with_2 as u64, total_unique as u64)
    );
    let _ = writeln!(
        out,
        "3+               | {:>12} | {:>5.2}%",
        nodes_with_3plus,
        pct(nodes_with_3plus as u64, total_unique as u64)
    );
    let _ = writeln!(out, "-----------------|--------------|------");
    let _ = writeln!(
        out,
    ...
    let _ = writeln!(
        out,
        "SECTION 3: ITEM-TYPE-COMBO HISTOGRAM (Top 20, only >=2-item junctions)"
    );
    let _ = writeln!(
        out,
        "----------------------------------------------------------------------"
    );
    let mut combos: Vec<(&Vec<u32>, &usize)> =
        combo_counts.iter().filter(|(c, _)| c.len() >= 2).collect();
    ...
    let _ = writeln!(
        out,
        "Junction nodes total (>=2 items)         : {:>10}",
        junction_total
    );
    let _ = writeln!(
        out,
        "  known in graph.json                    : {:>10}",
        junction_known_in_graph
    );
    let _ = writeln!(
        out,
        "    of which singletons (deg=0, no prefab): {:>10}",
        junction_singletons
    );
    let _ = writeln!(
        out,
        "    of which already connected           : {:>10}",
        junction_non_singletons
    );
    let _ = writeln!(
        out,
        "  unknown in graph.json (no node defined): {:>10}",
        junction_unknown_in_graph
    );
    let total_singletons_today = degree
        .iter()
        .filter(|(uid, d)| **d == 0 && !prefab_attached.contains(*uid))
        .count();
    let _ = writeln!(
        out,
        "Total singletons today                   : {:>10}",
        total_singletons_today
    );
    let _ = writeln!(
        out,
        "% of singletons covered by junctions     : {:>9.2}%",
    ...
    let _ = writeln!(
        out,
        "Junction-Pool size (nodes with >=2 items): {}",
        junction_total
    );
    let _ = writeln!(
        out,
        "Junction-Singletons reachable            : {}",
        junction_singletons
    );
    ...
    let _ = writeln!(
        out,
        "Junction edges between Sign/Model/MapOverlay/etc. would NOT be"
    );
    let _ = writeln!(
        out,
        "drivable roads. They are visual-anchor co-references. Implementing"
    );
    let _ = writeln!(
        out,
        "them as routing edges risks A* finding shortcuts the truck cannot"
    );
    let _ = writeln!(
        out,
        "physically traverse. Top-3 combos in Section 3 above show which"
    );
    let _ = writeln!(
        out,
        "type pairs dominate; combos like Sign+Model are most suspect, while"
    );
    let _ = writeln!(
        out,
        "BusStop+Sign or BusStop+Model are more plausible (BusStop carries"
    );
