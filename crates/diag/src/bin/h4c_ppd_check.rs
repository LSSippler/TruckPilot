...
fn walk(arc: &HashFsArchive) -> Vec<String> {
    ...
    while let Some(dir) = stack.pop() {
        ...
        for it in items {
            ...
            if it.is_dir {
                stack.push(child);
            } else if it.name.to_ascii_lowercase().ends_with(".ppd") {
                hits.push(child);
            }
        }
    }
...
fn main() {
    let ets2 = std::env::args()
        .skip_while(|a| a != "--ets2-dir")
        .nth(1)
        .unwrap_or_else(|| {
            r"C:\Program Files (x86)\Steam\steamapps\common\Euro Truck Simulator 2".into()
        });
    let mods = std::env::args()
        .skip_while(|a| a != "--mods-dir")
        .nth(1)
        .map(PathBuf::from)
        .unwrap_or_else(|| {
            dirs::document_dir()
    ...
    let mut per: Vec<(String, Vec<String>)> = order
        .entries
        .iter()
        .filter_map(|e| {
            HashFsArchive::open(&e.path)
                .ok()
                .map(|a| (e.name.clone(), walk(&a)))
        })
        .collect();
    ...
    buf.push_str(&format!(
        "H4c A1 QUICK-CHECK — PPD Availability\nArchives: {}\n\n",
        order.entries.len()
    ));
    ...
    File::create("outputs/h4c_a1_quick_check.txt")
        .unwrap()
        .write_all(buf.as_bytes())
        .unwrap();
    File::create("outputs/claude/h4c_a1_quick_check.txt")
        .unwrap()
        .write_all(buf.as_bytes())
        .unwrap();
    print!("{buf}");
