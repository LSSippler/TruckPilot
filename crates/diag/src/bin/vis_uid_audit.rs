...
struct GraphFile { nodes: Vec<NodeJson>, edges: Vec<EdgeJson> }
#[derive(serde::Deserialize)]
...
fn sweep_vis_layout(data: &[u8]) -> Option<(u32, Vec<u64>)> {
    ...
    for m in 0..=MAX_M {
        ...
        if vis_count_pos < 4 + 16 { continue; }
        let vis_count = u32::from_le_bytes(data[vis_count_pos..vis_count_pos + 4].try_into().unwrap());
        if vis_count as usize != m { continue; }
        let nodes_end = vis_count_pos;
        for n in 0..=MAX_N {
            let block = 4 + n * NODE_BYTES;
            if block > nodes_end { break; }
            let count_pos = nodes_end - block;
            if count_pos < 16 { break; }
            let count_at = u32::from_le_bytes(data[count_pos..count_pos + 4].try_into().unwrap());
            if count_at as usize != n { continue; }
            if n > 0 {
                let first_uid = u64::from_le_bytes(data[count_pos + 4..count_pos + 12].try_into().unwrap());
                if first_uid == 0 { continue; }
            }
...
fn main() {
    ...
    for (sid, (path, idx)) in sector_paths.iter().enumerate() {
        ...
        let uids = match sweep_vis_layout(&data) {
            Some((_, u)) => u,
            None => { sector_vis.push(Vec::new()); sectors_zero_m += 1; continue; }
        };
        let m = uids.len();
        if m == 0 { sectors_zero_m += 1; }
        else { sectors_with_vis += 1; }
        if m > 1000 { sectors_high_vis += 1; }
        if m > max_m { max_m = m; }
        s.total_vis += m;
        s.sum_for_mean += m;
        s.counts.push(m);
        ...
        for &uid in &uids {
            total_vis += 1;
            if uid == 0 { zero_uids += 1; unresolved += 1; continue; }
            ...
            *high_byte_hist.entry(top).or_insert(0) += 1;
            match (node_to_sector.get(&uid), node_to_arch.get(&uid)) {
                (Some(&owner_sid), Some(&owner_arch)) => {
                    if owner_sid == sid as u32 { same += 1; }
                    else { cross_sector += 1; }
                    if owner_arch != *idx { cross_archive += 1; }
                }
                _ => unresolved += 1,
            }
        }
    ...
    let singleton_set: HashSet<u64> = degree.iter()
        .filter(|(_, d)| **d == 0)
        .map(|(u, _)| *u)
        .collect();
    ...
    for (sid, _) in sector_paths.iter().enumerate() {
        ...
        let anchor = node_to_sector.iter()
            .find(|(_, s)| **s == sid as u32)
            .map(|(uid, _)| *uid);
        let Some(anchor) = anchor else { continue };
        for &uid in uids {
            if uid == 0 || uid == anchor { continue; }
            if !known_uids.contains(&uid) { continue; }
            hypothetical_edges += 1;
            if let (Some(&sa), Some(&sb)) = (node_to_sector.get(&anchor), node_to_sector.get(&uid)) {
                if sa != sb { hypothetical_cross_sector_edges += 1; }
            }
            if singleton_set.contains(&uid) { singletons_gaining_edge.insert(uid); }
            if singleton_set.contains(&anchor) { singletons_gaining_edge.insert(anchor); }
        }
    }
    ...
    let _ = writeln!(out, "------------------");
    let _ = writeln!(out, "Phase 5.13 (commit 0eddb8f8) ran on base_map.scs only (282 sectors).");
    let _ = writeln!(out, "Result: 954 vis_uids total, 100.0%% unresolved, 0%% same-sector,");
    let _ = writeln!(out, "0%% cross-sector. Decisive argument: parsed Node-UIDs live in");
    let _ = writeln!(out, "`0x0029XXXX...`, vis_uids in `0x45-0x5DXXXX...` — disjoint UID");
    let _ = writeln!(out, "namespaces. vis_uids are visibility/LOD asset hashes, not node");
    let _ = writeln!(out, "references. Reframe-C re-checks at multi-archive scale.");
    let _ = writeln!(out);

    let _ = writeln!(out, "GLOBAL VIS-UID STATISTICS");
    let _ = writeln!(out, "-------------------------");
    let _ = writeln!(out, "Total sectors                : {}", sector_paths.len());
    let _ = writeln!(out, "Sectors with M = 0           : {} ({:.1}%)",
        sectors_zero_m, pct(sectors_zero_m as u64, sector_paths.len() as u64));
    let _ = writeln!(out, "Sectors with M > 1000        : {} ({:.1}%)",
        sectors_high_vis, pct(sectors_high_vis as u64, sector_paths.len() as u64));
    let _ = writeln!(out, "Total vis_uids               : {}", total_vis);
    let _ = writeln!(out, "Mean per non-empty sector    : {:.1}",
        if sectors_with_vis > 0 { total_vis as f64 / sectors_with_vis as f64 } else { 0.0 });
    let _ = writeln!(out, "Max per sector               : {}", max_m);
    let _ = writeln!(out);

    let _ = writeln!(out, "PER-ARCHIVE BREAKDOWN");
    let _ = writeln!(out, "---------------------");
    let _ = writeln!(out, "Archive                    | Sectors |  Total VisUIDs | Mean | Median |   Max");
    ...
    for s in &entries {
        ...
        let median = if counts.is_empty() { 0 } else { counts[counts.len() / 2] };
        let mean = if s.sectors > 0 { s.sum_for_mean as f64 / s.sectors as f64 } else { 0.0 };
        let _ = writeln!(out, "{:<26} | {:>7} | {:>14} | {:>4.0} | {:>6} | {:>5}",
    ...
    let _ = writeln!(out, "------------------------------------------------");
    let _ = writeln!(out, "Total vis_uids               : {}", total_vis);
    let _ = writeln!(out, "  Zero (0x00)                : {} ({:.2}%)", zero_uids, pct(zero_uids, total_vis));
    let _ = writeln!(out, "  Same-sector resolved       : {} ({:.2}%)", same, pct(same, total_vis));
    let _ = writeln!(out, "  Cross-sector resolved      : {} ({:.2}%)", cross_sector, pct(cross_sector, total_vis));
    let _ = writeln!(out, "    of which cross-archive   : {} ({:.2}%)", cross_archive, pct(cross_archive, total_vis));
    let _ = writeln!(out, "  Unresolved                 : {} ({:.2}%)", unresolved, pct(unresolved, total_vis));
    let _ = writeln!(out);

    let _ = writeln!(out, "BYTE-PATTERN DISCRIMINATOR (top u8 of UID)");
    let _ = writeln!(out, "------------------------------------------");
    let _ = writeln!(out, "Phase 5.13 found Node-UIDs at 0x0029... and vis_uids at 0x45-0x5D...");
    let _ = writeln!(out, "If vis_uids share Node namespace, top byte should be 0x00 dominantly.");
    ...
    for (b, c) in hist.iter().take(10) {
        let _ = writeln!(out, "  0x{:02x}XXXXXXXXXXXXXX  : {:>10} ({:.2}%)",
            b, c, pct(*c, total_vis));
    }
    let _ = writeln!(out);
    ...
    let _ = writeln!(out, "----------------------------------------");
    let _ = writeln!(out, "Naive clique: per sector connect first-node anchor -> each vis_uid");
    let _ = writeln!(out, "that resolves in node_lookup.");
    let _ = writeln!(out, "  Singletons today            : {}", singletons_today);
    let _ = writeln!(out, "  Singletons that would gain edge: {} ({:.2}% of singletons)",
        singletons_gaining_edge.len(), pct(singletons_gaining_edge.len() as u64, singletons_today));
    let _ = writeln!(out, "  Hypothetical new edges      : {}", hypothetical_edges);
    let _ = writeln!(out, "    of which cross-sector     : {}", hypothetical_cross_sector_edges);
    let _ = writeln!(out);
    ...
    if singleton_reduction > 50.0 {
        let _ = writeln!(out, "H4a GO — vis_uid edges would reduce singletons by {:.1}%, exceeds", singleton_reduction);
        let _ = writeln!(out, "the 50% GO threshold. Proceed with edge-generation prototype.");
    } else if singleton_reduction > 10.0 {
        let _ = writeln!(out, "H4a REVIEW — singleton reduction {:.1}% is in the [10%, 50%] band.", singleton_reduction);
        let _ = writeln!(out, "Manual call: implementation effort vs Phase 5.13 namespace caveat.");
    } else if resolved_share < 5.0 {
        let _ = writeln!(out, "H4a SKIP — vis_uids do NOT resolve in node_lookup at scale");
        let _ = writeln!(out, "({:.2}% resolve, {:.2}% unresolved). Singleton reduction {:.2}%.",
            resolved_share, pct(unresolved, total_vis), singleton_reduction);
        let _ = writeln!(out, "Phase 5.13 verdict confirmed at multi-archive scale: vis_uids are");
        let _ = writeln!(out, "a disjoint UID namespace (visibility/LOD asset hashes), not node");
        let _ = writeln!(out, "references. Pivot to H4b (Single-Anchor-Junction-Detection) or");
        let _ = writeln!(out, "H4c (Prefab-PPD).");
    } else {
        let _ = writeln!(out, "H4a SKIP — singleton reduction only {:.2}% (resolve {:.2}%).", singleton_reduction, resolved_share);
        let _ = writeln!(out, "Below 10% threshold; implementation cost not justified.");
    }
