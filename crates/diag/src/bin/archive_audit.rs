...
const ITEM_TYPE_NAMES: &[(u32, &str)] = &[
    (1, "terrain"), (2, "buildings"), (3, "road"), (4, "prefab"),
    (5, "model"), (6, "company"), (7, "service"), (8, "cut_plane"),
    (12, "city"), (18, "map_overlay"), (19, "ferry"), (22, "garage"),
    (34, "trigger"), (35, "fuel_pump"), (36, "sign"), (37, "bus_stop"),
    (38, "traffic_area"), (39, "bezier_patch"), (41, "trajectory"),
    (42, "map_area"), (43, "far_model"), (44, "curve"), (46, "cutscene"),
    (48, "visibility_area"),
];

fn item_type_name(t: u32) -> String {
    ITEM_TYPE_NAMES.iter().find(|(k, _)| *k == t)
        .map(|(_, n)| n.to_string())
...
fn main() {
    ...
    for (_, idx) in &sector_paths {
        stats.entry(*idx).or_insert_with(|| ArchiveStats {
            name: archive_names[*idx].clone(), ..Default::default()
        });
    }
    ...
    for (path, idx) in &sector_paths {
        ...
        for r in &parsed.roads { s.roads.push((r.node_a, r.node_b)); }
        ...
        match (&report.failure, audit_consistent) {
            ...
            (Some(_), false) => { s.sectors_likely_sized += 1; }
            _ => {}
        }
    }
        ...
        for (a, b) in &s.roads {
            ...
            if let (Some(aa), Some(bb)) = (node_to_arch.get(a), node_to_arch.get(b)) {
                if aa != bb {
                    ...
                    if a_known && b_known { s.cross_archive_resolved += 1; }
                }
            }
        }
    ...
    let base_fail_rate = base_idx.map(|i| {
        let s = entries[i];
        if s.sectors_total > 0 {
            100.0 * s.sectors_audit_failed as f64 / s.sectors_total as f64
        } else { 0.0 }
    }).unwrap_or(0.0);
    let dlcs: Vec<&&ArchiveStats> = entries.iter().filter(|s| s.name != "base_map.scs").collect();
    let dlc_avg_fail = {
        let mut t = 0usize; let mut f = 0usize;
        for s in &dlcs { t += s.sectors_total; f += s.sectors_audit_failed; }
        if t > 0 { 100.0 * f as f64 / t as f64 } else { 0.0 }
    };
    ...
    let _ = writeln!(out, "Source: production load order ({} archives, {} owning .base sectors)",
        archives.len(), entries.len());
    let _ = writeln!(out, "============================================");
    let _ = writeln!(out);
    ...
    for s in &entries {
        let dom = s.failure_handlers.iter().max_by_key(|(_, c)| *c)
            .map(|(t, c)| format!("{} ({}) x{}", item_type_name(*t), t, c))
            .unwrap_or_else(|| "-".into());
        let pct = if s.sectors_total > 0 {
            100.0 * s.sectors_audit_failed as f64 / s.sectors_total as f64
        } else { 0.0 };
        let _ = writeln!(out, "{:<26} | {:>7} | {:>5} | {:>7} | {:>6} | {:>5.1}% | {}",
            s.name, s.sectors_total, s.sectors_parsed_ok, s.sectors_audit_failed,
            s.sectors_likely_sized, pct, dom);
    }
    let _ = writeln!(out);
    let _ = writeln!(out, "PER-ARCHIVE ITEM-COUNTS");
    let _ = writeln!(out, "-----------------------");
    let _ = writeln!(out, "Archive                    |   Roads | Prefabs |    Bldg | Curve | Terrain");
    for s in &entries {
        let _ = writeln!(out, "{:<26} | {:>7} | {:>7} | {:>7} | {:>5} | {:>7}",
            s.name, s.items_road, s.items_prefab, s.items_building,
            s.items_curve, s.items_terrain);
    }
    let _ = writeln!(out);
    let _ = writeln!(out, "PER-ARCHIVE NODE-CONNECTIVITY (H3 evidence)");
    let _ = writeln!(out, "-------------------------------------------");
    let _ = writeln!(out, "Archive                    | Defined | Singletons | Sgl-%");
    for s in &entries {
        let pct = if s.nodes_defined > 0 {
            100.0 * s.nodes_singletons as f64 / s.nodes_defined as f64
        } else { 0.0 };
        let _ = writeln!(out, "{:<26} | {:>7} | {:>10} | {:>5.1}%",
    ...
    for s in &entries {
        ...
        let cr_str = if cr >= 0.0 { format!("{:>8.1}%", cr) } else { "        -".into() };
        let _ = writeln!(out, "{:<26} | {:>7} | {:>8} | {:>7} | {:>6} | {:>9} | {}",
            s.name, s.items_road, s.roads_both_resolved, s.roads_one_unresolved,
            s.roads_unresolved, s.cross_archive_roads, cr_str);
    }
    let _ = writeln!(out);
    ...
    let dlcs_with_low_cross = dlcs.iter()
        .filter(|s| { let r = cross_resolve_rate(s); (0.0..50.0).contains(&r) })
        .count();
    let base_cross = base_idx.map(|i| cross_resolve_rate(entries[i])).unwrap_or(-1.0);
    let majority_dlcs_low_cross = !dlcs.is_empty() && dlcs_with_low_cross * 2 > dlcs.len();
    let h2_strength = if majority_dlcs_low_cross && base_cross > 90.0 && dlc_avg_fail < 2.0 * base_fail_rate.max(0.01) {
        if dlcs.iter().any(|s| { let r = cross_resolve_rate(s); (0.0..20.0).contains(&r) }) {
            "DECISIVE"
        } else { "STRONG" }
    } else if majority_dlcs_low_cross { "STRONG" } else { "WEAK" };

    let dlc_avg_singleton = {
        let mut tot = 0usize; let mut sgl = 0usize;
        for s in &dlcs { tot += s.nodes_defined; sgl += s.nodes_singletons; }
        if tot > 0 { 100.0 * sgl as f64 / tot as f64 } else { 0.0 }
    };
    let h3_strength = if dlc_avg_singleton > 60.0 { "STRONG" } else { "WEAK" };

    let _ = writeln!(out, "HYPOTHESIS RANKING");
    let _ = writeln!(out, "------------------");
    let _ = writeln!(out, "H1 (parse-failures higher in DLCs) : {}", h1_strength);
    let _ = writeln!(out, "  base fail-rate: {:.2}%, DLC-avg fail-rate: {:.2}%, ratio {:.1}x",
        base_fail_rate, dlc_avg_fail,
        if base_fail_rate > 0.01 { dlc_avg_fail / base_fail_rate } else { 0.0 });
    let _ = writeln!(out);
    let _ = writeln!(out, "H2 (cross-arch UID resolution broken) : {}", h2_strength);
    let _ = writeln!(out, "  base cross-resolve: {:.1}%, DLCs with <50% cross-resolve: {}/{}",
        base_cross, dlcs_with_low_cross, dlcs.len());
    let _ = writeln!(out);
    ...
    let dominant = [(h1_strength, "H1"), (h2_strength, "H2"), (h3_strength, "H3")]
        .iter()
        .max_by_key(|(s, _)| match *s { "DECISIVE" => 3, "STRONG" => 2, _ => 1 })
        .map(|(_, n)| *n).unwrap_or("-");
    ...
    match dominant {
        "H1" => {
            let _ = writeln!(out, "Strongest signal: H1. Hex-dump 10 failing sectors from worst DLC,");
            let _ = writeln!(out, "identify the failing item-type per Phase 5.20 BezierPatch playbook.");
            let worst = dlcs.iter()
            ...
            if let Some(w) = worst {
                let _ = writeln!(out, "Worst DLC: {} ({}/{} failed = {:.1}%).",
                    w.name, w.sectors_audit_failed, w.sectors_total,
                    100.0 * w.sectors_audit_failed as f64 / w.sectors_total as f64);
            }
        }
        "H2" => {
            let _ = writeln!(out, "Strongest signal: H2. DLC roads' cross-archive nodes don't resolve.");
            let _ = writeln!(out, "Likely cause: trailing-node blocks of DLC sectors are not parsed");
            let _ = writeln!(out, "(sector failed before reaching tail) so their nodes never enter");
            let _ = writeln!(out, "node_lookup. Fix upstream item-handler.");
        }
        "H3" => {
            let _ = writeln!(out, "Strongest signal: H3. DLC items parse and prefabs count normally");
            let _ = writeln!(out, "but cliques don't generate edges. Sample DLC prefab vs base prefab");
            let _ = writeln!(out, "and inspect connected_node_uids semantics + prefab descriptor lookup.");
        }
        _ => {
            let _ = writeln!(out, "All three hypotheses weak — fourth hypothesis required.");
            let _ = writeln!(out, "Candidates: prefab-template-token resolution (no .ppd parse yet);");
            let _ = writeln!(out, "vis_uids cross-sector edges (Phase 5.13 was rejected, may reopen).");
        }
    }
