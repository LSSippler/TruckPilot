//! Letterbox + ImageNet normalisation for UFLD v2 CULane input [1, 3, 320, 1600].

pub const INPUT_H: usize = 320;
pub const INPUT_W: usize = 1600;
...
pub struct LetterboxResult {
    /// Flat NCHW f32 buffer, shape [1, 3, INPUT_H, INPUT_W].
    pub nchw: Vec<f32>,
    pub meta: LetterboxMeta,
}

/// Letterbox `rgb` (packed RGB8, row-major) to `INPUT_W × INPUT_H`, then
...
pub fn letterbox_and_normalize(rgb: &[u8], src_w: u32, src_h: u32) -> Result<LetterboxResult, String> {
    ...
    let scale_x = INPUT_W as f32 / src_w as f32;
    let scale_y = INPUT_H as f32 / src_h as f32;
    ...
    let pad_x = ((INPUT_W as u32 - scaled_w) / 2) as f32;
    let pad_y = ((INPUT_H as u32 - scaled_h) / 2) as f32;
    ...
    let mut canvas = vec![0u8; INPUT_H * INPUT_W * 3];
    let px = pad_x as usize;
    let py = pad_y as usize;

    for row in 0..scaled_h as usize {
        let dst_row = py + row;
        if dst_row >= INPUT_H { break; }
        let src_off = row * scaled_w as usize * 3;
        let dst_off = (dst_row * INPUT_W + px) * 3;
        let copy_pixels = (scaled_w as usize).min(INPUT_W - px);
    ...
    let pixel_count = INPUT_H * INPUT_W;
    ...
    Ok(LetterboxResult {
        nchw,
        meta: LetterboxMeta { scale, pad_x, pad_y, orig_w: src_w, orig_h: src_h },
    })
}
...
mod tests {
    ...
    fn output_shape_matches_model_input() {
        let result = letterbox_and_normalize(&white_rgb(1920, 1080), 1920, 1080)
            .expect("letterbox ok");
        assert_eq!(result.nchw.len(), 3 * INPUT_H * INPUT_W);
    }

    #[test]
    fn square_input_has_symmetric_padding() {
        let meta = letterbox_and_normalize(&white_rgb(320, 320), 320, 320)
            .expect("ok")
            .meta;
        // Scale = min(1600/320, 320/320) = 1.0
        assert!((meta.scale - 1.0).abs() < 0.01, "scale={}", meta.scale);
        assert_eq!(meta.pad_y as u32, 0);
        // Horizontal padding = (1600-320)/2 = 640
        assert_eq!(meta.pad_x as u32, 640);
    }

    #[test]
    fn wrong_buffer_length_returns_error() {
        let result = letterbox_and_normalize(&[0u8; 10], 100, 100);
