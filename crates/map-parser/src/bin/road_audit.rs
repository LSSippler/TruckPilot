//! `truckpilot-road-audit` — Phase 5.15 (Road-handler) diagnostic.
//!
//! Walks every `.base` sector via [`audit_sector`], picks the first N sectors
//! where the **last successfully audited item is a Road** AND the audit failed
//! immediately afterwards (i.e. the road handler is suspected to have desynced
//! the cursor). For each such sector it dumps:
//!
//! * sector path, item count, road item-index, road `start_offset` (where the
//!   `item_type u32` was read), road `end_offset` (where the handler claims
//!   the road body ended).
//! * bytes-advance = `end_offset - start_offset` (should be `4 + 0x109 = 269`
//!   for the current 265-byte fixed-header parser).
//! * hex preview of 128 bytes starting at `start_offset + 4` (= road body).
//! * plausibility scan: at offsets `end_offset + Δ` for
//!   Δ in a small probe set, read a u32 and check whether it is a valid
//!   `item_type` (1..=48 set). The smallest non-negative Δ at which a valid
//!   type appears is the most likely off-by-N.
//!
//! Read-only. Output: `outputs/road_audit.txt`.
//!
//! Usage:
//!
//! ```powershell
//! cargo run --release --bin truckpilot-road-audit -- `
...
fn parse_args() -> Args {
    ...
    let mut output = PathBuf::from("outputs/road_audit.txt");
    ...
    while i < argv.len() {
        match argv[i].as_str() {
            ...
            "-h" | "--help" => {
                eprintln!(
                    "usage: truckpilot-road-audit --ets2-dir <PATH> [--output <FILE>] [--samples N]"
                );
                std::process::exit(0);
            }
            other => {
...
fn main() {
    ...
    let mut total_road_last = 0usize;
    // Global failure distribution across ALL failing sectors — gives the true
    // by-handler prioritisation (the road-as-last subset is biased by
    // road-item frequency in the item stream).
    ...
    eprintln!(
        "found {} sectors with road-as-last-successful, dumping first {}",
        total_road_last,
        samples.len()
    );
    ...
    let _ = writeln!(out, "# Phase 5.15 — Road-handler audit");
    let _ = writeln!(out);
    let _ = writeln!(out, "Archive:       {}", base_map.display());
    let _ = writeln!(out, "Total sectors: {}", sector_paths.len());
    let _ = writeln!(
        out,
        "Sectors with audit-failure AND last-success=road: {total_road_last}"
    );
    ...
    for (path, data, report) in &samples {
        ...
        let road_body = last.start_offset + 4;
        ...
        let _ = writeln!(out, "    road item index       : {}", last.index);
        let _ = writeln!(out, "    road start_offset     : {}", last.start_offset);
        let _ = writeln!(out, "    road end_offset       : {}", last.end_offset);
        ...
        let _ = writeln!(out, "    hex preview — 128 bytes from road body (skip type-tag):");
        out.push_str(&hex_block(data, road_body, 128));
        let _ = writeln!(out);
    ...
    match dominant {
        Some((d, n)) if (n as f64) / (total as f64) >= 0.8 => {
            ...
            let _ = writeln!(
                out,
                "→ Fix candidate: extend road handler to consume {d} extra bytes after the 265-byte fixed header."
            );
        }
        Some((d, n)) if n > 0 => {
            let _ = writeln!(
        ...
        _ => {
            let _ = writeln!(
                out,
                "→ NO CLEAR Δ — variable-length suffix in window > 32 bytes, or per-road dependent."
            );
            let _ = writeln!(
                out,
                "→ Re-run with wider Δ probe range, or reuse `road_full::RoadDataPayload` and compare consumed length per sample."
            );
        }
    }
