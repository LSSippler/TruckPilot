...
//
// Example:
//   <EngageButton state="off-ready" hotkey="F5" onClick={engage} />
//
// States — visually distinct via icon + label + color, not color alone.
//   off-ready    : enabled, neutral. Solid teal label/icon, surface-card bg.
//   off-disabled : disabled, muted. Tooltip explains why (preconditions).
//   engaging     : transient, spinner. Brand color, no pulse.
//   engaged      : active. Subtle brand-gradient border + soft pulse on a 2px ring.
//   disengaging  : transient, spinner reversed.
//   fault        : danger, attention-getting but not screaming.
...
export function EngageButton({
  ...
  return (
    <div className={cn("relative inline-flex", className)}>
      {/* Engaged ring — the only place the brand gradient appears on this button */}
      {isEngaged && (
        <span
          aria-hidden
      ...
      <button
        ...
        className={cn(
          ...
          // engaged — solid teal text + gradient ring above
